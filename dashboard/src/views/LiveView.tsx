    ...
    <section aria-labelledby="live-heading">
      ...
      <div className="card-grid compact">
        <article className="metric-card">
          <span>Evidence</span>
          <strong>{latest?.quality_label ?? "NONE"}</strong>
        </article>
        <article className="metric-card">
          <span>Fused risk</span>
          <strong>{latest?.fused_score?.toFixed(3) ?? "HELD"}</strong>
        </article>
        <article className="metric-card">
          <span>Confidence</span>
          <strong>{latest?.context_confidence.toFixed(3) ?? "N/A"}</strong>
        </article>
        <article className="metric-card">
          <span>Reason</span>
          <strong>{latest?.reason_code ?? "No decision"}</strong>
        </article>
      </div>
    </section>
  );
