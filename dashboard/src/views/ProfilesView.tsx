      ...
      <div className="profile-grid">
        {state.profiles.map((profile) => (
          <article className="detail-card" key={profile.user_id}>
            ...
            <p>
              {profile.user_state} · {profile.enrollment_windows} windows ·{" "}
              {profile.distinct_days} days
            </p>
            <p>Model: {profile.model_version ?? "not activated"}</p>
            <button
              disabled={!profile.model_version}
              onClick={() => onRollback(profile.user_id)}
            >
