  ...
  return (
    <section aria-labelledby="overview-heading">
      ...
      <div className="card-grid">
        ...
        <article
          className={`metric-card risk-${current?.risk_level.toLowerCase() ?? "unavailable"}`}
        >
      ...
      <article className="detail-card">
        ...
        <dl>
          <div>
            <dt>Smoothed risk</dt>
            <dd>{latest?.smoothed_score?.toFixed(3) ?? "No score"}</dd>
          </div>
          <div>
            <dt>Context confidence</dt>
            <dd>{latest?.context_confidence.toFixed(3) ?? "Unavailable"}</dd>
          </div>
          <div>
            <dt>Modalities</dt>
            <dd>{latest?.quality_label ?? "No evidence"}</dd>
          </div>
          <div>
            <dt>Enforcement</dt>
