...
PlatformMonotonicClock::PlatformMonotonicClock() noexcept {
    ...
    if (mach_timebase_info(&timebase) == KERN_SUCCESS && timebase.denom != 0) {
        scale_numerator_ = static_cast<std::uint64_t>(timebase.numer);
        scale_denominator_ = static_cast<std::uint64_t>(timebase.denom) * 1'000ULL;
    }
#elif defined(__linux__)
    scale_numerator_ = 1;
...
std::uint64_t PlatformMonotonicClock::now_us() const noexcept {
    ...
    return scale_ticks(
        static_cast<std::uint64_t>(counter.QuadPart), scale_numerator_, scale_denominator_);
#elif defined(__APPLE__)
    return valid() ? scale_ticks(mach_continuous_time(), scale_numerator_, scale_denominator_) : 0;
#elif defined(__linux__)
    timespec value{};
    if (!valid() || clock_gettime(CLOCK_MONOTONIC_RAW, &value) != 0 || value.tv_sec < 0 || value.tv_nsec < 0) {
        return 0;
    }
