...
**Document:** `PLAN.md`
**Plan Version:** v3.0 — Single-Participant Enrollment Baseline (supersedes v2.0; see ADR-014)
...
| v2.0 | — | Superseded | Restructured to phase-wise delegation only (no per-member allocation). Adds: formal architectural decision records, privacy-by-construction data model, staged data-collection programme, system state machine, evaluation methodology with impostor taxonomy, threat model, and AI-agent execution model. |
| v3.0 | 2026-09-09 | Current | **Major revision — evaluation methodology changed (Section 20.2).** *Current assumption:* an 8–15 person pilot cohort (Section 9.3) supplies zero-effort impostor data, so cross-participant FAR is available and was made a precondition of first-profile activation. *New evidence:* the delivered study has one participant, and `ml/evaluation/cross_evaluation.py` skips `other_id == user_id`, so a single-participant corpus produces no impostor score — making activation unreachable without either enrolling the attacker as a cohort member or fabricating `FAR = 0.0`. *Decision:* ADR-014 — initial enrollment is single-user; FRR is measured at the deployed operating threshold; FAR is recorded as explicitly UNMEASURED (`None`, never `0.0`) and is not an activation gate; the post-activation live attacker drill becomes the primary impostor evidence; attacker data is excluded from every corpus and every update candidate; model updates are disabled for this pilot. *Resulting plan update:* ADR-014 added to Section 6; Sections 9.3, 12.4, 13.2, 13.3 and 13.5 amended; `docs/adr/ADR-014-single-participant-enrollment-and-unseen-attacker.md` records the full decision. |
...
> **What counts toward `CALIBRATING` (ADR-014).** Only a scored window whose `profile_version` equals the currently active profile's version, and whose score shows at least one modality actually available. Rows written before activation carry `profile_version = "unavailable"` and can never satisfy the shadow period; nor can `INSUFFICIENT_DATA` windows, so an idle period cannot age a user into `ACTIVE`; nor can any window from a declared attacker-drill session. A superseded profile version restarts the shadow period rather than inheriting the previous one's credit.
...
**Context:** The Model Update Manager promotion gate (Section 12.1) admits new data into the training set of a profile that already exists. Gate G3 counts scored windows, and scoring requires an active model, so no first profile can ever satisfy it. Applying the promotion gate to enrollment therefore makes per-user models unbuildable from real data, which contradicts Section 5.3 and Section 9.6.

**Decision:** A user's first profile is admitted by a distinct enrollment admission gate requiring eligible provenance, active consent, a recorded enrollment, membership of a checksum-verified frozen corpus, the configured minimum window and distinct-day counts, and the absence of any active profile for that user. Every subsequent change to a user's training data continues to require the complete G1-G6 promotion gate, unchanged.

**Consequences:** Enrollment data is admitted against consent and corpus-integrity evidence rather than against risk-and-verification evidence that cannot exist yet. The poisoning surface the promotion gate protects is unchanged, because enrollment happens once per user, from an immutable frozen corpus, before any enforcement exists to subvert. The freeze manifest becomes a security boundary as well as a research one.

**Rejected alternative:** Redefining G3 as "windows in segment" for offline use. This weakens the promotion gate for every user and every update in order to solve a first-model problem, and hides the weakening inside a gate that still claims six passing conditions.

---

### ADR-014 — Single-Participant Enrollment With an Unseen Live Attacker

**Status:** Accepted, 2026-09-09. Supersedes the cohort assumption in Section 9.3 for this build. Full record: `docs/adr/ADR-014-single-participant-enrollment-and-unseen-attacker.md`.

**Context:** Section 9.3 planned an 8–15 person cohort in which every participant supplies zero-effort impostor (I1) data for every other participant's model, so a false-acceptance rate falls out of the corpus for free. The delivered study has **one participant**, and the intended experiment is different: the model is trained only on the legitimate user's own behaviour, there is no second enrolled participant, and after activation a classmate who has never contributed data acts as an **unseen attacker** in a live takeover on the legitimate user's own machine. The implementation, however, made cross-participant FAR a *precondition of activation* — `tools/enrollment/activate.py` refused unless both genuine and impostor VALIDATION scores existed, and `ml/evaluation/cross_evaluation.py` skips `other_id == user_id`. For N=1 that makes activation unreachable, and the only two ways to satisfy it were to enroll the attacker as a cohort member (inverting the experiment) or to fabricate `FAR = 0.0` (the exact failure Section 13.5 exists to prevent).

**Decision:**

1. Initial enrollment is single-user. Training, calibration, and validation of a first profile use **only** the legitimate user's own data, and no second enrolled participant is required for activation.
2. FAR/EER requires impostor data, and is therefore recorded as **UNMEASURED** (`ValidationMetrics.false_acceptance_rate = None`) for a first profile when no cohort exists. Never `0.0`; never estimated; never imputed.
3. **FAR is not an activation gate.** A first profile's acceptance criterion is the ADR-013 enrollment gate, because there is no prior profile to regress against.
4. **FRR is measured at the deployed operating threshold.** With no impostors there is no EER, so FRR is measured at the operating point the live system actually uses: `backend/app/models/service.py` maps a percentile to risk as `1 - percentile/100`, so `risk.medium_threshold` corresponds to the percentile `(1 - medium_threshold) * 100`, and the reported FRR is the fraction of the participant's own **held-out VALIDATION** windows that the live engine would place at or above MEDIUM. Genuine VALIDATION scores remain mandatory; their absence fails activation with `ENROLLMENT_VALIDATION_NO_GENUINE`.
5. The live attacker drill (Section 13.3) is **post-activation** and is never training data. Drill sessions are declared with `--drill-label`, recorded in `drill_sessions`, scored, risk-assessed, enforced, alerted and audited — and excluded from every corpus loader.
6. Attacker data is excluded from all future model-update candidates, from the automatic A1 login anchor, from context-confidence learning, and from enrollment/calibration progress. It is excluded from **nothing** in the live security path.
7. Optional multi-participant cohort evaluation may later measure FAR/EER; the I1 code remains present and unchanged, and runs automatically whenever impostor scores are found. It is not required for first activation.
8. Model updates remain **disabled** for this pilot (Section 12.4), unless explicitly re-enabled later.
9. The corpus is frozen **before** the attacker drill, so the guarantee is auditable by ordering and not merely enforced by a filter.

**Consequences — stated plainly, including what is given up:** no I1 cross-user matrix; no cross-user FAR, EER, ROC or DET; no per-user metric distribution; no cohort confidence intervals; the I2 informed-mimicry experiment of Section 13.2 is not run; E1 and E2 (Section 12.4) remain unrun because the update pipeline is disabled. Section 13.5's statistical-honesty requirements must be read as N=1 requirements. In their place, the live attacker drill becomes the project's **primary impostor evidence**, reported as a risk trajectory and a detection latency with the drill count and scored-window count adjacent to every claim — and explicitly **not** reported as a FAR, because a handful of live sessions does not satisfy the statistical requirements for one.

Note the deliberate asymmetry: an unmeasured FAR permits a **first activation** (`accepted` gates activation, and the criterion is the ADR-013 gate), but refuses an **update** (`accepted` there means "not worse than the active profile", which is undecidable without the measurement). `UpdateManager._validate` returns `VALIDATION_FAR_UNMEASURED`; `tools/updates/run.py` refuses earlier still, before training or artifact creation, with `UPDATE_REQUIRES_IMPOSTOR_COHORT`.

**Also decided here — training-admission scope.** One configuration key was answering two unrelated questions. `config/risk.*.yaml -> enrollment.min_windows / min_distinct_days` is the **live state-machine** policy, counted over the user's whole observed history, and is **unchanged** (`min_distinct_days: 3`). A separate, new `config/ml.*.yaml -> enrollment.min_train_windows / min_train_distinct_days` is the **training-admission** policy, scoped to the frozen TRAIN partition only. Conflating them silently turned a "3 distinct days" rule into an undocumented "6 collection days" requirement, because a 60/20/20 day-disjoint split gives TRAIN roughly half the collected days. The training-admission policy is: *a first profile must be trained on at least two distinct calendar days, so that a single day's posture, device position, or mood cannot alone define the identity baseline*; more than two is not justified until the Section 10.5 enrollment-length experiment (O3) is run.

**Rejected alternatives:** (a) enrolling a second participant to satisfy the activation code — it places the impostor inside the frozen corpus and changes the experiment to fit the implementation; (b) writing `FAR = 0.0` — a fabricated metric indistinguishable downstream from a real perfect score; (c) collecting additional days — resolves neither blocker, since no quantity of single-participant data produces an impostor; (d) lowering the enrollment day threshold so the existing corpus passes — the defect was scope conflation, not a value that was too high; (e) adding a drill flag to `FeatureWindow` or overloading `sessions.entry_auth_evidence` — the first forces protocol codegen and C++ changes and alters the freeze digest, the second corrupts G2's verification-anchor semantics.
...
> **Superseded for the delivered build by ADR-014.** The delivered study has **one participant**. Everything in this section describing a cohort — the 8–15 target, the quadratic FAR strengthening, and the cross-participant impostor supply — is retained as the original design intent and as the shape a future cohort round would take. It is **not** what was executed, and no part of it is a precondition of first-profile activation. The impostor evidence for this build is the post-activation live attacker drill (Section 13.3).
...
> **Status for the delivered build (ADR-014): the update pipeline is DISABLED and both experiments are UNRUN.**
>
> The capability exists in the architecture and in the codebase — the promotion gate, quarantine, scheduled cadence, validation, activation, and rollback are all implemented and tested. It is **not enabled for this pilot**, and no candidate is promoted.
>
> The reason is a consequence of ADR-014, not a defect. An update may only replace an active profile when it is shown not to have raised FAR; FAR needs impostor evidence; this project's only impostor evidence is the live attacker drill, which is excluded from every corpus by design. A candidate's FAR is therefore unmeasurable *by construction*. `UpdateManager._validate` refuses with `VALIDATION_FAR_UNMEASURED`, and `tools/updates/run.py` refuses earlier still — before any training runs or any artifact is written — with `UPDATE_REQUIRES_IMPOSTOR_COHORT`.
>
> Refusing is the correct outcome. The alternative would be relaxing `quarantine_days`, `retraining_cadence_days`, `regression_tolerance`, or the G1-G6 gates so that a disabled feature could run, which would weaken the exact safeguard this section claims as a contribution (P7: adaptation must be *earned*). **No gate, tolerance, or cadence is weakened.** E1 and E2 remain pending a cohort round.
...
> **Amended by ADR-014 for the delivered build.** I1 is "effectively free" only when a cohort exists. With one participant, `ml/evaluation/cross_evaluation.py` has no other user to score against and produces no impostor result at all — so **I1 is not available, and no cross-user FAR, EER, ROC, or DET is reported.** I2 as specified needs a cohort member and is likewise **not run**.
>
> The impostor evidence for this build is the **live attacker drill (Section 13.3)** — a classmate who has never contributed data, operating the legitimate user's machine after the corpus is frozen and the profile is `ACTIVE`. That is a genuinely unseen attacker, which is a stronger threat-model match than I1, and simultaneously a much smaller sample, which is a weaker statistical claim. Both facts must be reported together.
>
> The I1 and cross-evaluation code paths remain present, unchanged, and fully functional. If a cohort round is ever collected, both metrics are measured automatically — the code selects the path purely by whether impostor scores were found. Cohort evaluation is **optional research functionality, not an activation prerequisite**.
...
### 13.3.1 Promoted to primary impostor evidence (ADR-014)

For the delivered single-participant build this drill is not a supplement to the offline impostor results — it **is** the impostor result. Participant B is a classmate who has never contributed data and is never enrolled: no `user_id`, no consent record, no enrollment record, and no windows in any corpus.

**Non-negotiable ordering.** The corpus is frozen and the profile activated *before* the attacker touches the machine:

1. Stop collection. 2. **Freeze the corpus.** 3. Activate the profile. 4. Observe the shadow period. 5. Enable enforcement. 6. Start the backend **with `--drill-label`**. 7. Attacker operates. 8. Stop; export the risk trace. 9. **Never create a freeze that includes drill sessions.**

Ordering is what makes the guarantee auditable. The `drill_sessions` filter in `tools/collection/repository.py::load_window_summaries` and guardrail G12 enforce it mechanically; the ordering is what lets a reader verify it after the fact. The full operator procedure is `docs/pilot/attack-drill-protocol.md`.

**What the drill reports.** Attack session identifier; attack duration; number of scored windows; the full risk trajectory; highest risk reached; whether a soft challenge, an interruption, a lockout, and a reauthentication were each triggered; time and window count to each escalation; and the final enforcement outcome.

**What the drill does not report.** A false-acceptance rate. A small number of live sessions does not satisfy the statistical requirements for a FAR, and labelling the result one would be the same fabrication ADR-014 exists to prevent. This is **live security and robustness evidence**. Formal FAR/EER/ROC remains an optional cohort-evaluation capability (Section 13.2).

**What the drill suppresses,** for the whole labelled session: update-candidate submission, the automatic A1 login anchor, context-confidence learning, enrollment/calibration progress, and inclusion in every corpus loader. **What it does not suppress:** scoring, risk fusion, smoothing, risk-state transitions, the escalation ladder, challenges, interruption, lockout, reauthentication, enforcement, alerts, the audit log, and the WebSocket stream. Suppressing any of those would mean the drill was not testing the live system.
...
### 13.5.1 N=1 restatement (ADR-014)

The requirements above were written for a cohort. For the delivered build they must be read as follows, and the stricter reading applies:

- **Cohort size is 1.** State it adjacent to every headline metric, in those words. Per-user distributions, cohort spreads, and bootstrap confidence intervals over users are **not computable and must not be presented**, in any form, including as a single-user "range".
- **FRR is a genuine measurement** on the participant's own held-out VALIDATION partition, reported together with the operating point it was measured at (`risk.medium_threshold` on the calibrated risk scale), the window count, and the collection dates.
- **FAR is reported as UNMEASURED, with the reason**, wherever it would otherwise appear. Never as `0.0`, never as "approximately zero", never omitted silently. The stored `ValidationReport.operating_point` string carries the reason next to the number and should be quoted in the report.
- **Drill results carry N.** The number of drills, the number of scored windows per drill, and the identity of the attacker as "one unseen classmate" appear adjacent to every drill figure.
- **TRAIN composition is disclosed**, including partial days. The delivered TRAIN partition is two calendar days, one of which is partial.
- **The participant is the author.** This is a self-collected, non-blind sample of one — a stronger disclosure than Section 13.5's team-member caveat, and it must be made in the same sentence as any accuracy claim.
- **Nothing here is general-population validation, and nothing here is a cohort result.** The defensible claim is *"demonstrated end to end on one participant's own machine, with one live unseen-attacker drill."*
...
| O3 | Minimum enrollment duration and `min_windows` / `min_distinct_days` | Phase 4 (experiment 10.5) — **still open.** ADR-014 split this into two policies: the live `risk.enrollment.*` thresholds (unchanged) and the new TRAIN-scoped `ml.enrollment.min_train_*`. The latter is set provisionally to 2 distinct days on the stated policy argument, explicitly *pending* this experiment. |
