...
def _sample_positive(rng: np.random.Generator, mean: float, std: float, floor: float = 1000.0) -> float:
...
def generate_mouse_stream(
    ...
    *,
    ...
    while t < t_end:
        speed = _sample_positive(rng, profile.mean_mouse_speed_px_s, profile.std_mouse_speed_px_s, floor=0.0)
        ...
        if t >= next_scroll_t:
            events.append(
                MouseEvent(
                    ...
                    x=x,
                    y=y,
                    scroll_dy=float(rng.choice([-1.0, 1.0])),
                    device_class=device_class,
                    app_id=app_id,
                    seq=seq.next(),
                )
...
def generate_context_stream(
    ...
    *,
    ...
    """Generate deterministic APP_FOCUS_CHANGE events biased toward the user's preferred category."""
    other_categories = [c for c in AppCategory if c != profile.preferred_app_category]
    events: list[ContextEvent] = []
    t = float(t_start_us)
    t_end = t_start_us + duration_us
    events.append(
        ContextEvent(t_capture_us=int(t), app_id=app_id, category=profile.preferred_app_category, seq=seq.next())
    )
    while True:
        ...
        events.append(ContextEvent(t_capture_us=int(t), app_id=app_id, category=category, seq=seq.next()))
    return events
...
def generate_segment(
    ...
    *,
    ...
    mouse_events = generate_mouse_stream(profile, rng, duration_us=duration_us, t_start_us=t_start_us, seq=seq)
    context_events = generate_context_stream(profile, rng, duration_us=duration_us, t_start_us=t_start_us, seq=seq)
