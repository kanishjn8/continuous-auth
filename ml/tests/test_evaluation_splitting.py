...
def test_day_disjoint_split_separates_by_calendar_day(ml_config):
    windows = generate_multiday_user_windows(
        "u1", 1, ml_config, num_days=4, segments_per_day=1, segment_minutes=10
    )
...
def test_day_disjoint_split_rejects_unknown_day(ml_config):
    windows = generate_multiday_user_windows(
        "u1", 1, ml_config, num_days=3, segments_per_day=1, segment_minutes=10
    )
...
def test_day_disjoint_split_rejects_covering_every_day(ml_config):
    windows = generate_multiday_user_windows(
        "u1", 1, ml_config, num_days=2, segments_per_day=1, segment_minutes=10
    )
    days = distinct_days(windows)
...
def test_leave_one_day_out_requires_at_least_two_days(ml_config):
    windows = generate_multiday_user_windows(
        "u1", 1, ml_config, num_days=1, segments_per_day=1, segment_minutes=10
    )
...
def test_enrollment_length_subsets_grow_with_day_count(ml_config):
    windows = generate_multiday_user_windows(
        "u1", 1, ml_config, num_days=6, segments_per_day=1, segment_minutes=10
    )
    days = distinct_days(windows)
    holdout = [days[-1]]
    ...
    for _n_days, subset in subsets.items():
